...
export async function getAssetList(wallet) {
  ...
  async function getBalance() {
    try {
      ...
      // Return default balance if API call fails
      return [{ balance: "0" }];
    }
  }
  ...
  return assetList.map(asset => [asset.policy_id, asset.asset_name]);
    ...
    // Create ADA item with default balance of 0 if necessary
  const adaAmount = balance && balance.length > 0 ? (Number(balance[0].balance) / Math.pow(10, 6)).toFixed(6) : "0.000000";
